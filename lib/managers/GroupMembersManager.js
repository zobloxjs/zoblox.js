...
class GroupMembersManager {
  ...
  async fetch({ roleId, limit, sortOrder, cursor } = {}) {
    try {
      ...
      const { data: Members } = await this.zoblox.session.get(this.zoblox.Routes.groups.players(this.id, roleId, limit, sortOrder, cursor));
      return Members;
    } catch (e) {
  ...
  async get(userId) {
    ...
    return new GroupMember(Group, userId, this.group, this.zoblox);
  }
};
