...
class GroupsManager {
  ...
  async search({ keyword, prioritizeExactMatch, firstGroup, limit, cursor }) {
    try {
      ...
      const { data: Groups } = await this.zoblox.session.get(this.zoblox.Routes.groups.search(keyword, prioritizeExactMatch, limit, cursor));
  ...
  async get(groupId) {
    try {
      const { data: Group } = await this.zoblox.session.get(this.zoblox.Routes.groups.group(groupId));
      if (Group.shout) {
