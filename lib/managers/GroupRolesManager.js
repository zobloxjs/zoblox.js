...
class GroupRolesManager {
  ...
  async permissions() {
    try {
      const { data: RolesPermissions } = await this.zoblox.session.get(this.zoblox.Routes.groups.rolesPermissions(this.id));
  ...
  async fetch() {
    try {
      const { data: Roles } = await this.zoblox.session.get(this.zoblox.Routes.groups.roles(this.id));
      return Roles;
    } catch (e) {
      if (e.response) throw new Error(`${e.response.status} ${e.response.data.errors.map(e => e.message)}`);
      if (!e.response) throw new Error(e.message);
  ...
  async get(roleId) {
    try {
      const { data: RolePermissions } = await this.zoblox.session.get(this.zoblox.Routes.groups.rolePermissions(this.id, roleId));
